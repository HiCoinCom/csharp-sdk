...
{
  /// <summary>
  /// Base WaaS API class with invoke methods
  /// </summary>
  public abstract class WaasApi
  {
    protected readonly WaasConfig Config;
    protected readonly IDataCrypto DataCrypto;
    protected readonly ILogger? Logger;

    protected WaasApi(WaasConfig config, IDataCrypto dataCrypto, ILogger? logger = null)
    {
    ...
    /// <summary>
    /// Invoke a WaaS API endpoint with dictionary parameters
    /// </summary>
    ...
    protected async Task<Result<T>> InvokeAsync<T>(ApiUri apiUri, Dictionary<string, object> bizContent)
    {
      try
      {
      ...
      {
        Logger?.LogError(ex, "API invoke failed: {ApiUri}", apiUri.Value);
        return Result<T>.Error(-1, ex.Message);
      }
    }

    /// <summary>
    /// Invoke a WaaS API endpoint with JSON string
    /// </summary>
    /// <param name="apiUri">The API URI enum value</param>
    /// <param name="bizContent">The business content as JSON string</param>
    /// <returns>The API response result</returns>
    protected async Task<Result<T>> InvokeAsync<T>(ApiUri apiUri, string bizContent)
    {
      try
      {
        if (Config.EnableLog)
        {
          Logger?.LogInformation("Request API: {ApiUri}, Content: {Content}", apiUri.Value, bizContent);
        }
        ...
        var args = BaseArgs.Create(Config.AppId, encryptedData);
        ...
        if (!string.IsNullOrEmpty(result.Data))
        {
          ...
          if (Config.EnableLog)
          {
            Logger?.LogInformation("Response Data: {Data}", decryptedData);
          }
    ...
    /// <summary>
    /// Invoke a WaaS API endpoint and return raw string result
    /// </summary>
    protected async Task<Result<string>> InvokeRawAsync(ApiUri apiUri, Dictionary<string, object> bizContent)
    {
      try
      {
        var jsonContent = JsonConvert.SerializeObject(bizContent);
    ...
    /// Invoke a WaaS API endpoint and return raw string result
    /// </summary>
    protected async Task<Result<string>> InvokeRawAsync(ApiUri apiUri, string bizContent)
    {
      try
      {
        if (Config.EnableLog)
